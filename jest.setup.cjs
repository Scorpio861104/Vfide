...
jest.mock('framer-motion', () => {
  ...
  const renderTag = (tag) => ({ children, ...props }) =>
    React.createElement(tag, props, resolveMotionChildren(children));
  ...
  return {
    motion: {
      ...
      li: renderTag('li'),
      a: renderTag('a'),
      img: (props) => React.createElement('img', { alt: '', ...props }),
      svg: renderTag('svg'),
      path: (props) => React.createElement('path', props),
      table: renderTag('table'),
      thead: renderTag('thead'),
      tbody: renderTag('tbody'),
      tr: renderTag('tr'),
      th: renderTag('th'),
      td: renderTag('td'),
      section: renderTag('section'),
      article: renderTag('article'),
      aside: renderTag('aside'),
      nav: renderTag('nav'),
      header: renderTag('header'),
      footer: renderTag('footer'),
      form: renderTag('form'),
...
jest.mock('lucide-react', () => {
  const createIcon = (name) => {
    const IconComponent = (props) => React.createElement('span', { 'data-testid': `icon-${name}`, ...props });
...
jest.mock('@/lib/mobile', () => ({
  ...
  ResponsiveSection: ({ children }) => React.createElement('section', { 'data-testid': 'responsive-section' }, children),
...
jest.mock('@/components/profile/AvatarUpload', () => ({
  AvatarUploadCompact: () => React.createElement('div', { 'data-testid': 'avatar-upload' }, 'Avatar'),
}))
