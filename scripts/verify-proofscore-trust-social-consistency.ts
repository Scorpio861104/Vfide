...
async function main() {
  ...
  const socialFactory = new ContractFactory(socialArtifact.abi as any, socialArtifact.bytecode, dao);
  ...
  if (active[0].length !== 1 || !active[0][0] || active[0][0].toLowerCase() !== endorserAddress.toLowerCase()) {
  ...
  await expectRevert(() => seer.connect(operator).reward(subjectAddress, 1, "op_reward_over_daily"));
