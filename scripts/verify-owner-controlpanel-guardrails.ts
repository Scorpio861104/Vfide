...
async function main() {
  ...
  const ocpArtifact = loadArtifact("artifacts/contracts/OwnerControlPanel.sol/OwnerControlPanel.json");
  ...
  const ecoFactory = new ContractFactory(ecosystemArtifact.abi as any, ecosystemArtifact.bytecode, owner);
  ...
  await expectRevert(async () =>
    ocp.autoSwap_configure(await other.getAddress(), await owner.getAddress(), true, 400, { gasLimit: 5_000_000 })
  );
  ...
  await expectRevert(() => ocp.ecosystem_configureAutoWorkPayout(true, 1, 1, 1, { gasLimit: 5_000_000 }));
  ...
  await (
    await ocp.ecosystem_configureAutoWorkPayout(
      true,
      oneEth,
      halfEth,
      quarterEth
  ...
  await expectRevert(async () =>
    ocp.autoSwap_quickSetupUSDC(await owner.getAddress(), await other.getAddress(), { gasLimit: 5_000_000 })
  );
