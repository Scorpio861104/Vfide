...
async function main() {
  ...
  await expectRevert(async () =>
    timelock.connect(nonDaoSigner).queueTx(await timelock.getAddress(), 0, '0x')
  );

  // Guardrail: cannot queue a zero target operation.
  await expectRevert(() => timelock.queueTx('0x0000000000000000000000000000000000000000', 0, '0x'));
  ...
  const firstId = await queueAndGetId(timelock, await timelock.getAddress(), 0n, delayData);

  // Timelock must enforce the delay before execution.
  await expectRevert(async () => timelock.execute(firstId, { gasLimit: 5_000_000 }));
  ...
  const secondId = await queueAndGetId(timelock, await timelock.getAddress(), 0n, setAdminData);

  await (await timelock.cancel(secondId)).wait();

  await expectRevert(async () => timelock.execute(secondId, { gasLimit: 5_000_000 }));
