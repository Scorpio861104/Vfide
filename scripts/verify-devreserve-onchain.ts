...
const UNLOCK = 1_666_666n * 10n ** 18n;
...
async function deployFixture() {
  ...
  const startTs = now + 1n;
  await setTime(startTs);
  await vestingVault.connect(beneficiary).setVestingStart(startTs);

  return { beneficiary, vaultAddress, startTs, token, vaultHub, vestingVault };
}
...
async function verifyFullVestingPayout(fx: any) {
  const VESTING = 30n * 60n * 24n * 60n * 60n;
