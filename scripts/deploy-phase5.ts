...
async function main() {
  ...
  console.log('Phase 5 deploy network:', networkName);
  ...
  if (deployed.MerchantRegistry) {
    deployed.VFIDECommerce = deployed.MerchantRegistry;
  }

  writeDeploymentBook(networkName, deployed);
  console.log(`\nPhase 5 deployment complete. Saved addresses to .deployments/${networkName}.json`);
}
