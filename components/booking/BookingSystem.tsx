...
export function BookingManager({ bookings = [], slots: _slots = [], onConfirm, onCancel, onComplete }: { bookings: Booking[]; slots: TimeSlot[]; onConfirm?: (id: string) => void; onCancel?: (id: string) => void; onComplete?: (id: string) => void; }) {
  ...
  return (<div className="space-y-6">
    <div className="flex gap-2">{(['upcoming','past','slots'] as const).map(v=>(<button key={v} onClick={()=>setView(v)} className={`px-4 py-2 rounded-xl text-sm font-bold capitalize ${view===v?'bg-accent/20 text-accent border border-accent/30':'bg-white/5 text-gray-400 border border-white/10'}`}>{v}{v==='upcoming'&&upcoming.length>0?` (${upcoming.length})`:''}</button>))}</div>
    {view!=='slots'&&<div className="space-y-2">{(view==='upcoming'?upcoming:past).map(b=>(<div key={b.id} className="p-4 bg-white/3 border border-white/5 rounded-xl"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><User size={16} className="text-gray-400"/><span className="text-white font-medium text-sm">{b.customerName}</span><span className={`px-2 py-0.5 rounded text-xs font-bold bg-${colors[b.status]}-500/20 text-${colors[b.status]}-400 capitalize`}>{b.status}</span></div><span className="text-accent font-mono font-bold">{formatCurrency(b.price)}</span></div><div className="text-gray-500 text-xs">{b.service} · {formatDate(b.date,'medium')} · {b.startTime}–{b.endTime}</div>{b.status==='pending'&&<div className="flex gap-2 mt-2"><button onClick={()=>onConfirm?.(b.id)} className="px-3 py-1.5 bg-emerald-500/20 text-emerald-400 rounded-lg text-xs font-bold">Confirm</button><button onClick={()=>onCancel?.(b.id)} className="px-3 py-1.5 bg-red-500/20 text-red-400 rounded-lg text-xs font-bold">Cancel</button></div>}{b.status==='confirmed'&&<button onClick={()=>onComplete?.(b.id)} className="mt-2 px-3 py-1.5 bg-accent/20 text-accent rounded-lg text-xs font-bold">Mark Complete</button>}</div>))}</div>}
