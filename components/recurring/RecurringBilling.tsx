...
export function RecurringBilling({ subscriptions = [], onPause, onResume, onCancel: _onCancel }: { subscriptions: Subscription[]; onPause?: (id: string) => void; onResume?: (id: string) => void; onCancel?: (id: string) => void; }) {
  ...
  return (<div className="space-y-6">
    <div className="grid grid-cols-2 gap-4"><div className="bg-accent/5 border border-accent/15 rounded-xl p-4"><div className="text-gray-400 text-xs mb-1">Active Subscriptions</div><div className="text-cyan-400 font-bold text-lg">{stats.active}</div></div><div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-4"><div className="text-gray-400 text-xs mb-1">Monthly Recurring</div><div className="text-emerald-400 font-bold text-lg font-mono">{formatCurrency(stats.mrr)}</div></div></div>
