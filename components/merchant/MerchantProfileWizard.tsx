      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="basics" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-gray-600 focus:border-cyan-500/50 focus:outline-none"
                />
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-gray-600 focus:border-cyan-500/50 focus:outline-none resize-none"
                />
                <div className="mt-1 flex items-center justify-between">
            ...
            <div className="mt-8 flex justify-end">
              <button
                ...
                disabled={!step1Valid}
                className="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-5 py-2.5 font-semibold text-white transition-colors hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
                    ...
                    <div className="flex gap-2">
                      ...
                      {avatarPreview && avatarFile && (
                        <button
                          ...
                          type="button"
                          className="inline-flex items-center gap-1.5 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-300 hover:bg-red-500/20"
                        >
                          <X size={14} /> Remove
                        </button>
                      )}
                ...
                <div className="space-y-2">
                  {links.map((link, i) => (
                    <div key={i} className="flex gap-2">
                      <input
                        ...
                        className="w-32 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-gray-600 focus:border-cyan-500/50 focus:outline-none"
                      />
                      <input
                        type="url"
                        value={link.url}
                        ...
                        placeholder="https://..."
                        className="flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-gray-600 focus:border-cyan-500/50 focus:outline-none"
                      />
                      <button
                        onClick={() => removeLink(i)}
                        type="button"
                        className="rounded-xl border border-white/10 bg-white/5 px-2 text-gray-400 hover:bg-red-500/10 hover:text-red-400"
                      >
                        <X size={14} />
                      </button>
                    </div>
                  ...
                  {links.length < MAX_LINKS && (
                    <button
                      onClick={addLink}
                      type="button"
                      className="rounded-xl border border-dashed border-white/10 bg-white/2 px-3 py-2 text-sm text-gray-400 hover:border-cyan-500/30 hover:text-cyan-300"
                    >
                      + Add link
                    </button>
                  )}
        ...
        {step === 'review' && (
          <motion.div key="review" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            ...
            <div className="mb-6 rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {bio && <p className="mt-1 text-sm text-gray-400">{bio}</p>}
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {links
                        ...
                        .map((l, i) => (
                          <span
                            key={i}
                            className="inline-flex items-center gap-1 rounded-md bg-white/5 px-2 py-1 text-xs text-cyan-300"
                          >
            ...
            {/* Privacy disclosure — required acknowledgment per spec discussions */}
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-gray-300 hover:bg-white/10 disabled:opacity-50"
              >
                <ArrowLeft size={14} /> Back
              </button>
              <button
                ...
                disabled={!canSubmit}
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-6 py-2.5 font-semibold text-white transition-colors hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Check size={16} />
                {isSubmitting
                  ? 'Publishing…'
