...
const leaderboardsData: Record<'proofScore' | 'badges', LeaderboardEntry[]> = {
  proofScore: [],
  badges: [],
};

const streaksData: ActivityStreak[] = [];
...
export function SocialInteractions({ currentUserId: _currentUserId = 'current_user' }: SocialInteractionsProps) {
  ...
  const unreadCount = useMemo(() => {
    return messagesData.filter((m) => !m.read && !readMessages.has(m.id)).length;
  }, [readMessages]);
  ...
  const tabs = [
    { key: 'requests', label: 'Friend Requests', icon: '👥', count: friendRequestsData.length - acceptedRequests.size },
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'requests' && (
            <motion.div
              ...
              className="space-y-4"
            >
              {friendRequestsData.map((request, idx) => (
                <motion.div
                  key={request.id}
          ...
          {activeTab === 'leaderboards' && (
            ...
            >
              ...
              <div className="space-y-3">
                {leaderboardsData[leaderboardType].map((entry, idx) => (
                  <motion.div
                    key={entry.user.id}
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
