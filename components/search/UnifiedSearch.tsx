      ...
      <div className="relative">
        ...
        {isLoading && (
          <div className="absolute right-12 top-1/2 -translate-y-1/2">
            <div className="w-4 h-4 border-2 border-accent/30 border-t-cyan-400 rounded-full animate-spin" />
          </div>
        )}
      </div>
              ...
              <div className="p-3 border-b border-white/5">
                ...
                {results.merchants.map((m, i) => (
                  <button key={m.merchant_address} onClick={() => goToMerchant(m.slug)}
                    className={`w-full text-left px-3 py-3 rounded-xl transition-colors flex items-center gap-3 ${activeIndex === i ? 'bg-cyan-500/10 border border-accent/20' : 'hover:bg-white/5 border border-transparent'}`}>
                ...
                {results.products.map((p, i) => {
                  ...
                  return (
                    <button key={`${p.slug}-${i}`} onClick={() => goToProduct(p.merchant_slug, p.slug)}
                      className={`w-full text-left px-3 py-2.5 rounded-xl transition-colors flex items-center gap-3 ${activeIndex === gi ? 'bg-cyan-500/10 border border-accent/20' : 'hover:bg-white/5 border border-transparent'}`}>
