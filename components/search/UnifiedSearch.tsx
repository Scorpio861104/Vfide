...
export function UnifiedSearch({
  ...
  const s = useUnifiedSearch(containerRef);
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <div className="relative">
        ...
        <input ref={s.inputRef} type="text" value={s.query} onChange={(e) => s.handleInput(e.target.value)}
          onFocus={() => s.setIsOpen(true)} onKeyDown={s.handleKeyDown} placeholder={placeholder} autoFocus={autoFocus}
        ...
        {s.query && (
          <button onClick={s.clearQuery} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-cyan-400 transition-colors">
      ...
      <AnimatePresence>
        {s.showDropdown && (
          <motion.div initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }}
            ...
            {/* Recent */}
            {s.query.length === 0 && s.recentSearches.length > 0 && (
              <div className="p-3">
                ...
                {s.recentSearches.map((term) => (
                  <button key={term} onClick={() => { s.handleInput(term); s.goToSearch(term); }}
            ...
            {s.results.merchants.length > 0 && (
              <div className="p-3 border-b border-white/5">
                ...
                {s.results.merchants.map((m, i) => (
                  <button key={m.merchant_address} onClick={() => s.goToMerchant(m.slug)}
                    className={`w-full text-left px-3 py-3 rounded-xl transition-colors flex items-center gap-3 ${s.activeIndex === i ? 'bg-cyan-500/10 border border-cyan-500/20' : 'hover:bg-white/5 border border-transparent'}`}>
            ...
            {s.results.products.length > 0 && (
              <div className="p-3">
                ...
                {s.results.products.map((p, i) => {
                  const gi = s.results.merchants.length + i;
                  return (
                    <button key={`${p.slug}-${i}`} onClick={() => s.goToProduct(p.merchant_slug, p.slug)}
                      className={`w-full text-left px-3 py-2.5 rounded-xl transition-colors flex items-center gap-3 ${s.activeIndex === gi ? 'bg-cyan-500/10 border border-cyan-500/20' : 'hover:bg-white/5 border border-transparent'}`}>
            ...
            {s.query.trim().length >= 2 && (
              <button onClick={() => s.goToSearch(s.query.trim())}
                ...
                <Search size={14} />Search all results for &ldquo;{s.query.trim()}&rdquo;<ArrowRight size={14} />
              </button>
            )}

            {s.isLoading && !s.hasResults && <div className="px-4 py-8 text-center text-gray-500 text-sm">Searching...</div>}
            {!s.isLoading && s.query.length >= 2 && !s.hasResults && <div className="px-4 py-8 text-center text-gray-500 text-sm">No merchants or products found for &ldquo;{s.query}&rdquo;</div>}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
