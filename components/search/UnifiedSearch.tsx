          ...
          <m.div initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }}
                ...
                {results.products.map((p, i) => {
                  ...
                  return (
                    <button key={`${p.slug}-${i}`} onClick={() => goToProduct(p.merchant_slug, p.slug)}
                      className={`w-full text-left px-3 py-2.5 rounded-xl transition-colors flex items-center gap-3 ${activeIndex === gi ? 'bg-accent/10 border border-accent/20' : 'hover:bg-white/5 border border-transparent'}`}>
            ...
            {query.trim().length >= 2 && (
              <button onClick={() => goToSearch(query.trim())}
                className="w-full px-4 py-3 border-t border-white/5 text-sm text-accent hover:bg-accent/5 transition-colors flex items-center justify-center gap-2">
