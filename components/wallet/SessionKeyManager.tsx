...
export function SessionKeyManager({ targetContracts = [], className = '' }: SessionKeyManagerProps) {
  ...
  return (
    <div className={className}>
      ...
      <AnimatePresence>
        {showCreateDialog && (
          <CreateSessionDialog isOpen={showCreateDialog} onClose={() => setShowCreateDialog(false)}
            onSubmit={(params: CreateSessionParams) => createSession(params)} targetContracts={targetContracts} />
        )}
      </AnimatePresence>
    </div>
  );
