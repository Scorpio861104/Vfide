...
- [x] H-11 — Contract size violations (EIP-170 deploy blocker) — FIXED: `scripts/verify-contract-size-buffer.ts` now reports all compiled runtime bytecode under 24,576 (notably `MerchantPortal` 24,398 and `EcosystemVault` 23,846). `scripts/deploy-all.ts` enforces an EIP-170 runtime bytecode preflight and hard-fails deployment if any contract in the deployment set exceeds 24,576 bytes.
