...
contract VFIDEToken is Ownable {
    ...
    bool public emergencyRouterBypass = false;    // emergency override for router failures
    ...
    /// Emergency DAO safety mechanism to prevent token lockup if router fails
    function setEmergencyDAO(address _dao) external onlyOwner {
        require(_dao != address(0), "VF: zero DAO");
        emergencyDAO = _dao;
        emit EmergencyDAOSet(_dao);
        _log("emergency_dao_set");
    }

    /// Emergency override to bypass router requirement if router fails after policyLock
    function setEmergencyRouterBypass(bool enabled, string calldata reason) external {
        ...
        _log(enabled ? "emergency_bypass_on" : "emergency_bypass_off");
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        // If policyLocked and emergencyRouterBypass enabled, skip router to prevent lockup
        bool skipRouter = policyLocked && emergencyRouterBypass;
        if (address(burnRouter) != address(0) && !(systemExempt[from] || systemExempt[to]) && !skipRouter) {
    ...
    function _locked(address vault) internal view returns (bool) {
        // Safe-default: treat call failures or invalid responses as locked (true)
        (bool ok, bytes memory d) = address(securityHub).staticcall(abi.encodeWithSelector(ISecurityHub.isLocked.selector, vault));
        if (!ok || d.length < 32) return true;  // call failed or invalid response -> locked
        return abi.decode(d, (bool));           // decode actual lock status
    }
