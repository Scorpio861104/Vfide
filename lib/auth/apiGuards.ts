/**
 * API auth/authz guards (extracted from middleware.ts).
 *
 * These were previously defined in lib/auth/middleware.ts, but next/jest's SWC
 * transformer treats ANY file named `middleware.ts` as a Next.js Edge Middleware and
 * rewrites its exports under test (stripping the named guard exports). The real
 * `next build` does not do this, but it made the guards untestable via jest. Housing the
 * guards here (a non-`middleware` filename) keeps them unit-testable; middleware.ts
 * re-exports everything (`export *`) so the ~130 route importers are unaffected.
 *
 * Logic is unchanged from the original middleware.ts.
 */
