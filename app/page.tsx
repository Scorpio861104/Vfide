  ...
  return (
    <>
      <div className="min-h-screen bg-zinc-950 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 max-w-6xl">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
                <h1 className="text-5xl md:text-6xl font-bold text-white mb-6 leading-tight">Keep what you <span className="text-cyan-400">earn</span></h1>
                <p className="text-xl text-gray-400 mb-8 max-w-lg">Zero merchant fees. Non-custodial vaults. Trust-scored payments. Built for everyone the platforms forgot.</p>
                <div className="flex flex-wrap gap-4">
                  <Link href="/merchant/setup" className="px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl font-bold text-lg hover:scale-[1.02] transition-transform flex items-center gap-2">Start selling <ArrowRight size={20} /></Link>
                  <Link href="/marketplace" className="px-8 py-4 bg-white/5 border border-white/10 text-white rounded-xl font-bold text-lg hover:bg-white/10 transition-colors">Browse marketplace</Link>
                </div>
              </motion.div>
              <HeroVisualization />
            </div>
        ...
        <section className="py-16 border-y border-white/5"><div className="container mx-auto px-4 max-w-6xl"><div className="grid grid-cols-2 md:grid-cols-4 gap-8"><StatItem value={0} label="Merchant Fees" suffix="%" color="cyan" /><StatItem value={10000} label="Max ProofScore" color="amber" /><StatItem value={35} label="Burn Rate" suffix="%" color="emerald" /><StatItem value={20} label="To Charity" suffix="%" color="pink" /></div></div></section>
