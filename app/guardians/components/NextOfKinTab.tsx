      ...
      {inheritanceRequest && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {isUserGuardian && <button onClick={() => void safeExec(approveInheritance, 'Approval submitted.', 'Failed')} disabled={isWritePending || !hasVault || !isUserGuardianMature} className="w-full py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-bold shadow-lg shadow-green-500/25 disabled:opacity-50">Approve Current Vault (Guardian)</button>}
            {isUserGuardian && <button onClick={() => void safeExec(guardianCancelInheritance, 'Cancellation submitted.', 'Failed')} disabled={isWritePending || !hasVault || !isUserGuardianMature} className="w-full py-3 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl font-bold shadow-lg shadow-red-500/25 disabled:opacity-50">Cancel Current Vault Vote (Guardian)</button>}
            {isNextOfKin && <button onClick={() => void safeExec(finalizeInheritance, 'Finalization submitted.', 'Failed')} disabled={isWritePending || !hasVault} className="w-full py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl font-bold shadow-lg shadow-cyan-500/25 disabled:opacity-50">Finalize Current Vault (Next of Kin)</button>}
            {isOwner && <button onClick={() => void safeExec(denyInheritance, 'Denial submitted.', 'Failed')} disabled={isWritePending || !hasVault} className="w-full py-3 border border-red-500/60 text-red-300 rounded-xl font-bold hover:bg-red-500/10 disabled:opacity-50">Deny Current Vault (Owner)</button>}
            {isOwner && <button onClick={() => void safeExec(cancelInheritance, 'Cancellation submitted.', 'Failed')} disabled={isWritePending || !hasVault} className="w-full py-3 border border-red-500/60 text-red-300 rounded-xl font-bold hover:bg-red-500/10 disabled:opacity-50">Cancel Current Vault (Owner)</button>}
          </div>
        </motion.div>
      )}
