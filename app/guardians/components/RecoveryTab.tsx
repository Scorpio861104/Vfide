...
export function RecoveryTab({ isConnected }: { isConnected: boolean }) {
  ...
  const {
    recoveryStatus, guardianCount, isUserGuardian, isUserGuardianMature,
  ...
  const withNotice = async (fn: () => Promise<void>, successMsg: string, failMsg: string) => {
    clearNotices();
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-4xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <h2 className="text-xl font-bold text-cyan-400 mb-4 flex items-center gap-3"><Key size={24} />Chain of Return</h2>
        ...
        <div className="bg-black/30 rounded-xl p-4 border border-white/10">
          <p className="text-gray-400 text-sm">
            Requires 2/3 guardian approval + 7-day maturity. Recovery expires after 30 days.
            Owner can cancel fraudulent requests at any time.
          </p>
        </div>
      </motion.div>
      ...
      {recoveryStatus.isActive ? (
        <RecoveryActivePanel
          ...
          hasVault={hasVault}
          onFinalize={() => void withNotice(() => finalizeRecovery(), 'Recovery finalized.', 'Failed to finalize recovery')}
          onCancel={() => void withNotice(() => cancelRecovery(), 'Recovery cancelled.', 'Failed to cancel recovery')}
          onApprove={() => void withNotice(() => approveRecovery(), 'Approval submitted.', 'Failed to approve recovery')}
        />
      ) : (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <p className="text-gray-400 mb-4">No active recovery request. If you lost your wallet, start a recovery to regain access via a new address.</p>
          {!isUserGuardian && <p className="text-amber-300 text-sm mb-4">Your current wallet is not configured as a guardian for this vault.</p>}
          {isUserGuardian && !isUserGuardianMature && <p className="text-amber-300 text-sm mb-4">You are a guardian, but still in maturity period.</p>}
          {guardianCount < 2 && <p className="text-amber-300 text-sm mb-4">This vault has fewer than 2 guardians. Add at least one more.</p>}
            ...
            {showRequestForm && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                ...
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-4">
                  <p className="text-yellow-400 text-sm"><strong>Who can request recovery?</strong> A mature guardian selected by the vault holder.</p>
                </div>
                <div>
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  ...
                  disabled={isWritePending || !hasVault || !isAddress(newAddress) || !isUserGuardian || !isUserGuardianMature || guardianCount < 2}
                ...
                <p className="text-gray-500 text-xs text-center">Recovery expires after 30 days. Finalization requires the minimum 7-day timelock.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
