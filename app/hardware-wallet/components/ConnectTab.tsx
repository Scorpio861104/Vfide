          ...
          {(['ledger', 'trezor'] as DeviceType[]).map((d) => (
            <button
              ...
              className={`p-4 rounded-xl border text-sm font-semibold capitalize transition-colors ${
                deviceType === d
                  ? 'bg-cyan-500/20 border-accent/40 text-cyan-400'
          ...
          <>
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 flex-1 justify-center px-4 py-2.5 rounded-lg bg-accent/20 hover:bg-accent/30 text-cyan-400 text-sm font-semibold transition-colors disabled:opacity-50"
              >
                {status === 'connecting' ? (
