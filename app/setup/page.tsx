...
const NETWORK_CONFIG = {
  ...
  rpc: targetNetwork.rpcUrls?.default?.http?.[0] || 'https://sepolia.base.org',
  chainId: targetNetwork.id,
  chainIdHex: `0x${targetNetwork.id.toString(16).toUpperCase()}`,
  symbol: targetNetwork.nativeCurrency.symbol,
  explorer: targetNetwork.blockExplorers?.default?.url || 'https://sepolia.basescan.org',
}
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <p className="text-zinc-400 mb-8">
          Complete these 3 steps to start using VFIDE on {NETWORK_CONFIG.name}
        </p>
        ...
        {step3Done && (
          <div className="mb-8 p-6 bg-green-500/10 border border-green-500 rounded-xl text-center">
            ...
            <p className="text-zinc-300 mb-4">Connected to {NETWORK_CONFIG.name} with {ethBalance.toFixed(4)} ETH</p>
            <Link 
              href="/token-launch"
        ...
        <div className={`mb-6 p-6 rounded-xl border-2 ${step2Done ? 'border-green-500 bg-green-500/5' : step1Done ? 'border-cyan-500 bg-cyan-500/5' : 'border-zinc-700 bg-zinc-900'}`}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold">Add {NETWORK_CONFIG.name} Network</h2>
          </div>

          {step2Done ? (
              <p className="text-green-400">✓ Connected to {NETWORK_CONFIG.name}</p>
          ) : (
                ...
                >
                  {addingNetwork ? (
                    ...
                    <>
                      ...
                      Add {NETWORK_CONFIG.name} to MetaMask
                    </>
                  )}
                </button>
