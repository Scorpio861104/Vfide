...
import { Heart, Key, UserPlus } from 'lucide-react';
          ...
          <GlassCard className="p-6" hover={false}>
            <div className="flex items-start justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-white mb-2 flex items-center gap-2">
                  ...
                  {cardBoundMode ? 'CardBound Guardian Setup' : 'Chain of Return (Recovery)'}
                </h2>
                <p className="text-white/60 text-sm">
                  {cardBoundMode
                    ? 'CardBound vaults use guardian-backed wallet rotation and queue protections instead of legacy inheritance.'
              ...
              <div className="px-4 py-2 bg-cyan-500/20 border border-cyan-500/30 rounded-xl text-center">
                <div className="text-cyan-400 text-xs font-bold">{cardBoundMode ? 'CARD BOUND' : '30-DAY EXPIRY'}</div>
              </div>
            </div>
            ...
            <div className="p-4 bg-white/5 rounded-xl mb-4">
              {guardianCount > 0 ? (
                ...
                <div className="text-amber-400 text-sm text-center py-2">
                  {cardBoundMode ? '⚠️ No guardians configured yet. Complete guardian setup to enable recovery and transfer protections.' : '⚠️ No guardians. Next of Kin will have instant access.'}
                </div>
              )}
            </div>
            ...
            {isOwner && (
              cardBoundMode ? (
                <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-4 text-sm text-cyan-100">
                  Manage guardian setup and post-setup guardian changes from the Guardians dashboard, which supports CardBound timelocks and setup completion.
                </div>
              ) : (
