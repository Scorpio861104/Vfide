...
const claimWeeklyChallengeSchema = z.object({
  challengeId: z.union([z.number().int().positive(), z.string().regex(/^\d+$/)]),
...
async function postHandler(request: NextRequest, user: JWTPayload) {
  ...
  let body: z.infer<typeof claimWeeklyChallengeSchema>;
  try {
    const rawBody = await request.json();
    const parsed = claimWeeklyChallengeSchema.safeParse(rawBody);
    if (!parsed.success) {
    ...
    logger.debug('[Quests Weekly Claim] Invalid JSON body', error);
    return NextResponse.json(
