...
function statusBadge(status?: ProposalStatus): { bg: string; text: string; Icon: typeof Clock } {
  switch (status) {
    case ProposalStatus.Active:
      return { bg: 'bg-accent/10 border-accent/30', text: 'text-accent', Icon: Sparkles };
...
export function ProposalCard({
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-6 hover:border-accent transition-colors">
      <div className="flex items-start justify-between mb-4 gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <div className="inline-block px-3 py-1 bg-accent/20 border border-accent rounded text-accent text-sm font-bold">
              {prop.type}
            </div>
            {status !== undefined && (
            ...
            {viewer?.hasVoted && isActive && (
              <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border bg-accent/10 border-accent/30 text-accent">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <button
          ...
          className="px-4 py-2 bg-zinc-900 border border-zinc-700 text-zinc-400 rounded-lg hover:text-accent hover:border-accent transition-colors"
        >
          View Details
        </button>
      </div>
