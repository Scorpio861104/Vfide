...
contract EmergencyControl {
    ...
    function committeeVote(bool halt, string calldata reason) external {
        ...
        if (halt) {
            ...
            if (approvalsHalt >= threshold) {
                ...
                uint8 voteCount = approvalsHalt;
                lastToggleTs = uint64(block.timestamp);
                approvalsHalt = 0;
                approvalsUnhalt = 0;
                
                // INTERACTIONS: External calls after state updates
                emit CommitteeVote(msg.sender, true, voteCount, reason);
                _logEv(msg.sender, "ec_vote_halt", voteCount, reason);
            ...
            if (approvalsUnhalt >= threshold) {
                ...
                // EFFECTS: Update state before external interactions (CEI pattern)
                uint8 voteCount = approvalsUnhalt;
                lastToggleTs = uint64(block.timestamp);
                approvalsHalt = 0;
                approvalsUnhalt = 0;
                
                // INTERACTIONS: External calls after state updates
                emit CommitteeVote(msg.sender, false, voteCount, reason);
                _logEv(msg.sender, "ec_vote_unhalt", voteCount, reason);
    ...
    // ───────────────────────────────── Internals
    // Note: Vote resets now happen inline in committeeVote() to prevent reentrancy
