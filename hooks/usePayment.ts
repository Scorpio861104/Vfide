...
import { getContractAddresses, getContractConfigurationError, isConfiguredContractAddress } from '@/lib/contracts';
...
export function usePayment() {
  ...
  const pay = useCallback(async (options: UsePaymentOptions): Promise<PaymentResult | null> => {
    ...
    const { merchantAddress, amount } = options;
    ...
    try {
      ...
      if (typeof balance === 'bigint' && balance < totalRequired) {
  ...
  }, [address, publicClient, writeContractAsync, balance, hasBurnRouterConfig, hasTokenConfig, chainId, contractAddresses]);
