...
if (process.env.WS_TLS_CERT && process.env.WS_TLS_KEY) {
    ...
    if (IS_PRODUCTION && process.env.WS_ALLOW_INSECURE_HTTP !== 'true') {
        throw new Error('[ws] Refusing to start in production without TLS. ' +
            'Configure WS_TLS_CERT + WS_TLS_KEY, terminate TLS upstream, or set WS_ALLOW_INSECURE_HTTP=true to override.');
    }
    server = http.createServer();
    console.warn('[ws] Plain HTTP mode — TLS MUST be terminated upstream (nginx/Caddy). ' +
 ...
 */
function chatTopic(addrA, addrB) {
...
function loadTopicAclSnapshot(path) {
    try {
        ...
        console.error(`[ws] Failed to load topic ACL snapshot from ${path}: ${err.message}`);
        return null;
    }
}
...
function isAuthorizedForTopic(client, topic, refreshForSubscribe = false) {
    ...
    if (topic.startsWith('chat.')) {
        ...
        if (!addrRe.test(a) || !addrRe.test(b) || a >= b)
            return false;
...
async function authenticateClient(client, token) {
...
wss.on('connection', (ws, _req) => {
    ...
    if (!client.isAuthenticated) {
        client.authTimeoutTimer = setTimeout(() => {
            if (!client.isAuthenticated && client.readyState === ws_1.WebSocket.OPEN) {
    ...
    client.send(JSON.stringify({
        ...
        payload: {
            ...
            serverTime: Date.now(),
            protocolVersion: schema_1.CURRENT_PROTOCOL_VERSION,
    ...
    ws.on('message', async (raw) => {
        ...
        const result = (0, schema_1.parseMessage)(parsed);
        if (!result.success) {
            sendError(ws, 'INVALID_SCHEMA', result.error.issues[0]?.message || 'Schema validation failed');
            return;
        }
        const msg = result.data;
        const messageVersion = msg.v ?? schema_1.CURRENT_PROTOCOL_VERSION;
        if (messageVersion !== schema_1.CURRENT_PROTOCOL_VERSION) {
            sendError(ws, 'INVALID_PROTOCOL_VERSION', `Unsupported protocol version ${messageVersion}. Expected ${schema_1.CURRENT_PROTOCOL_VERSION}`);
            return;
        }
        ...
        switch (msg.type) {
            case 'auth':
                ...
                try {
                    ...
                    sendError(ws, 'AUTH_FAILED', err.message || 'Invalid token');
