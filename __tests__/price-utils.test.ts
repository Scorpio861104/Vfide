...
describe('price-utils', () => {
  describe('vfideToUsd', () => {
    ...
    it('returns $0.00 for NaN', () => {
      expect(vfideToUsd('not-a-number', 0.05)).toBe('$0.00')
    })

    it('formats small amounts correctly', () => {
      // Values < $1 use 4 decimal places
      expect(vfideToUsd(1, 0.05)).toBe('$0.0500') // 1 * 0.05 = $0.05
      expect(vfideToUsd(5, 0.05)).toBe('$0.2500') // 5 * 0.05 = $0.25
    })

    it('formats amounts >= $1 with 2 decimals', () => {
      expect(vfideToUsd(20, 0.05)).toBe('$1.00')   // 20 * 0.05 = $1.00
      expect(vfideToUsd(100, 0.05)).toBe('$5.00')  // 100 * 0.05 = $5.00
      expect(vfideToUsd(200, 0.05)).toBe('$10.00') // 200 * 0.05 = $10.00
    })

    it('formats thousands with K suffix', () => {
      expect(vfideToUsd(20000, 0.05)).toBe('$1.0K') // 20000 * 0.05 = $1000
      expect(vfideToUsd(100000, 0.05)).toBe('$5.0K') // 100000 * 0.05 = $5000
    })

    it('formats millions with M suffix', () => {
      expect(vfideToUsd(20000000, 0.05)).toBe('$1.00M') // 20M * 0.05 = $1M
      expect(vfideToUsd(100000000, 0.05)).toBe('$5.00M')
    })
    ...
    it('handles very small amounts with 4 decimals', () => {
      expect(vfideToUsd(0.1, 0.05)).toBe('$0.0050') // 0.1 * 0.05 = $0.005
    })
  })
  ...
  describe('formatVfideWithUsd', () => {
    ...
    it('handles string input', () => {
      const result = formatVfideWithUsd('5000', 0.05)
    ...
    it('handles large amounts', () => {
      const result = formatVfideWithUsd(20000000, 0.05)
