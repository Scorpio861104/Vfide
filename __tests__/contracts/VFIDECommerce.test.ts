...
describe('VFIDECommerce Contract', () => {
  ...
  describe('Payment Processing', () => {
    ...
    it('should get payment details', async () => {
      ...
      expect(await mockContractRead({ functionName: 'getPaymentDetails', args: [1] })).toEqual({
        amount: parseEther('100'),
        merchant,
        buyer,
        timestamp: 123456,
    ...
    it('should reject invalid payment amount', async () => {
      ...
      await expect(
        mockContractWrite({ functionName: 'processPayment', args: [merchant, parseEther('0')] })
  ...
  describe('Merchant Management', () => {
    it('should register merchant', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(
        await mockContractWrite({ functionName: 'registerMerchant', args: ['Store Name'] })
      ).toBe('0xhash');
    });
    ...
    it('should deactivate merchant', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(
        await mockContractWrite({ functionName: 'deactivateMerchant', args: [merchant] })
      ).toBe('0xhash');
    });
  });
  ...
  describe('Dispute Resolution', () => {
    it('should open dispute', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(
        await mockContractWrite({ functionName: 'openDispute', args: [1, 'Item not received'] })
      ).toBe('0xhash');
    });

    it('should resolve dispute', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'resolveDispute', args: [1, true] })).toBe(
        '0xhash'
      );
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle duplicate merchant registration', async () => {
      ...
      await expect(
        mockContractWrite({ functionName: 'registerMerchant', args: ['Store'] })
    ...
    it('should handle refund of non-existent payment', async () => {
      ...
      await expect(
        mockContractWrite({ functionName: 'refundPayment', args: [9999] })
