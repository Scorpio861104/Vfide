...
describe('VFIDEEnterpriseGateway Contract', () => {
  ...
  describe('Enterprise Registration', () => {
    it('should register enterprise', async () => {
      ...
      expect(
        await mockContractWrite({
          ...
          args: ['Company Inc', 'admin@company.com'],
        })
      ).toBe('0xhash');
    });
    ...
    it('should get enterprise info', async () => {
      ...
      expect(
        await mockContractRead({ functionName: 'getEnterpriseInfo', args: [enterprise] })
      ).toEqual({ name: 'Company Inc', active: true, tier: 2 });
    });

    it('should upgrade enterprise tier', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(
        await mockContractWrite({ functionName: 'upgradeEnterpriseTier', args: [enterprise, 3] })
      ).toBe('0xhash');
    });
    ...
    it('should reject duplicate registration', async () => {
      ...
      await expect(
        mockContractWrite({ functionName: 'registerEnterprise', args: ['Company', 'email'] })
  ...
  describe('API Key Management', () => {
    it('should generate API key', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'generateAPIKey', args: [enterprise] })).toBe(
        '0xhash'
      );
    });
    ...
    it('should revoke API key', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'revokeAPIKey', args: ['api_key_123'] })).toBe(
        '0xhash'
      );
    });
    ...
    it('should validate API key', async () => {
      ...
      expect(
        await mockContractRead({ functionName: 'validateAPIKey', args: ['api_key_123'] })
      ).toBe(true);
    });
  });
  ...
  describe('Billing and Usage', () => {
    it('should record API usage', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(
        await mockContractWrite({ functionName: 'recordAPIUsage', args: [enterprise, 100] })
      ).toBe('0xhash');
    });

    it('should get usage stats', async () => {
      mockContractRead.mockResolvedValueOnce({ calls: 1000, cost: parseEther('10') });
      expect(await mockContractRead({ functionName: 'getUsageStats', args: [enterprise] })).toEqual(
        { calls: 1000, cost: parseEther('10') }
      );
    });

    it('should process billing', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'processBilling', args: [enterprise] })).toBe(
        '0xhash'
      );
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should reject enabled stable settlement with zero router', async () => {
      ...
      await expect(
        mockContractWrite({
          ...
          args: ['0x0000000000000000000000000000000000000000' as Address, enterprise, true, 100],
        })
      ).rejects.toThrow('ENT_Zero');
    });
    ...
    it('should handle invalid email', async () => {
      ...
      await expect(
        mockContractWrite({ functionName: 'registerEnterprise', args: ['Company', 'invalid'] })
    ...
    it('should handle tier downgrade attempt', async () => {
      ...
      await expect(
        mockContractWrite({ functionName: 'upgradeEnterpriseTier', args: [enterprise, 1] })
      ).rejects.toThrow('CannotDowngradeTier');
    });
  });
});
