...
describe('TempVault Contract', () => {
  ...
  describe('Vault Operations', () => {
    ...
    it('should get balance', async () => {
      ...
      expect(await mockContractRead({ functionName: 'balanceOf', args: [user1] })).toBe(parseEther('100'));
    });

    it('should reject withdrawal exceeding balance', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('InsufficientBalance'));
      await expect(mockContractWrite({ functionName: 'withdraw', args: [parseEther('1000')] })).rejects.toThrow('InsufficientBalance');
    });
  ...
  describe('Temporary Storage', () => {
    it('should store temporary data', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'storeTempData', args: ['key', 'value'] })).toBe('0xhash');
    });
    ...
    it('should clear temporary data', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'clearTempData', args: ['key'] })).toBe('0xhash');
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle empty key', async () => {
      ...
      await expect(mockContractWrite({ functionName: 'storeTempData', args: ['', 'value'] })).rejects.toThrow('EmptyKey');
    });
