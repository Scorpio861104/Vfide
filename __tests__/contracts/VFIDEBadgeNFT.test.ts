...
describe('VFIDEBadgeNFT Contract', () => {
  ...
  describe('NFT Operations', () => {
    ...
    it('should get token URI', async () => {
      ...
      expect(await mockContractRead({ functionName: 'tokenURI', args: [1] })).toBe(
        'ipfs://badge/1'
      );
    });
    ...
    it('should reject mint to zero address', async () => {
      ...
      ).rejects.toThrow('MintToZeroAddress');
    });

    it('should reject transfer of non-existent token', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('TokenDoesNotExist'));
      await expect(
        mockContractWrite({ functionName: 'transferFrom', args: [user1, owner, 9999] })
      ).rejects.toThrow('TokenDoesNotExist');
    });
  });
  ...
  describe('Badge Metadata', () => {
    it('should get badge info', async () => {
      ...
      expect(await mockContractRead({ functionName: 'getBadgeInfo', args: [1] })).toEqual({
        name: 'Pioneer',
        rarity: 'Legendary',
      });
    });
    ...
    it('should set badge URI', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(
        await mockContractWrite({ functionName: 'setBadgeURI', args: [1, 'ipfs://new'] })
      ).toBe('0xhash');
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle invalid token ID', async () => {
      ...
      await expect(mockContractRead({ functionName: 'tokenURI', args: [0] })).rejects.toThrow(
        'InvalidTokenId'
      );
    });
    ...
    it('should handle unauthorized mint', async () => {
      ...
      await expect(mockContractWrite({ functionName: 'mint', args: [user1, 1] })).rejects.toThrow(
        'Unauthorized'
      );
    });
  });
