...
describe('MainstreamPayments Contract', () => {
  ...
  describe('Payment Processing', () => {
    it('should process payment', async () => {
      ...
      expect(await mockContractWrite({ functionName: 'processPayment', args: [merchant, parseEther('100'), '0x'] })).toBe('0xhash');
    });

    it('should reject invalid payment amount', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('InvalidAmount'));
      await expect(mockContractWrite({ functionName: 'processPayment', args: [merchant, parseEther('0'), '0x'] })).rejects.toThrow('InvalidAmount');
    });

    it('should reject payment to invalid merchant', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('InvalidMerchant'));
      await expect(mockContractWrite({ functionName: 'processPayment', args: ['0x0000000000000000000000000000000000000000' as Address, parseEther('100'), '0x'] })).rejects.toThrow('InvalidMerchant');
    });
  });
  ...
  describe('Merchant Management', () => {
    it('should register merchant', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'registerMerchant', args: [merchant] })).toBe('0xhash');
    });
    ...
    it('should get merchant info', async () => {
      ...
      expect(await mockContractRead({ functionName: 'getMerchantInfo', args: [merchant] })).toEqual({ active: true, totalVolume: parseEther('10000') });
    });
  });
  ...
  describe('Fee Management', () => {
    ...
    it('should calculate fee', async () => {
      ...
      expect(await mockContractRead({ functionName: 'calculateFee', args: [parseEther('100')] })).toBe(parseEther('2.5'));
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle zero address merchant', async () => {
      ...
      await expect(mockContractWrite({ functionName: 'registerMerchant', args: ['0x0000000000000000000000000000000000000000' as Address] })).rejects.toThrow('ZeroAddress');
    });
    ...
    it('should handle duplicate merchant registration', async () => {
      ...
      await expect(mockContractWrite({ functionName: 'registerMerchant', args: [merchant] })).rejects.toThrow('AlreadyRegistered');
    });

    it('should handle payment exceeding limits', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('ExceedsLimit'));
      await expect(mockContractWrite({ functionName: 'processPayment', args: [merchant, parseEther('1000000'), '0x'] })).rejects.toThrow('ExceedsLimit');
    });
  });
});
