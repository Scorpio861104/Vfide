...
describe('VaultRecoveryClaim Contract', () => {
  ...
  describe('Recovery Claims', () => {
    it('should submit recovery claim', async () => {
      ...
      expect(await mockContractWrite({ functionName: 'submitRecoveryClaim', args: [owner, 'Lost access'] })).toBe('0xhash');
    });

    it('should get claim details', async () => {
      mockContractRead.mockResolvedValueOnce({ claimant, vault: owner, reason: 'Lost access', status: 0 });
      expect(await mockContractRead({ functionName: 'getClaimDetails', args: [1] })).toEqual({ claimant, vault: owner, reason: 'Lost access', status: 0 });
    });
    ...
    it('should reject claim', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'rejectClaim', args: [1, 'Invalid proof'] })).toBe('0xhash');
    });
    ...
    it('should reject duplicate claim', async () => {
      ...
      await expect(mockContractWrite({ functionName: 'submitRecoveryClaim', args: [owner, 'Lost access'] })).rejects.toThrow('ClaimPending');
    });

    it('should reject unauthorized approval', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('Unauthorized'));
      await expect(mockContractWrite({ functionName: 'approveClaim', args: [1] })).rejects.toThrow('Unauthorized');
    });
  });
  ...
  describe('Proof Submission', () => {
    it('should submit proof', async () => {
      mockContractWrite.mockResolvedValueOnce('0xhash');
      expect(await mockContractWrite({ functionName: 'submitProof', args: [1, 'ipfs://proof'] })).toBe('0xhash');
    });
  ...
  describe('Timelock Management', () => {
    it('should get claim timelock', async () => {
      ...
      expect(await mockContractRead({ functionName: 'getClaimTimelock', args: [1] })).toBe(7 * 86400);
    });
    ...
    it('should reject claim before timelock', async () => {
      ...
      await expect(mockContractWrite({ functionName: 'executeClaim', args: [1] })).rejects.toThrow('TimelockActive');
    });
  });
  ...
  describe('Claim Execution', () => {
    ...
    it('should reject execution of unapproved claim', async () => {
      ...
      await expect(mockContractWrite({ functionName: 'executeClaim', args: [1] })).rejects.toThrow('ClaimNotApproved');
    });

    it('should reject execution of expired claim', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('ClaimExpired'));
      await expect(mockContractWrite({ functionName: 'executeClaim', args: [1] })).rejects.toThrow('ClaimExpired');
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle invalid claim ID', async () => {
      ...
      await expect(mockContractRead({ functionName: 'getClaimDetails', args: [9999] })).rejects.toThrow('InvalidClaimId');
    });

    it('should handle empty reason', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('EmptyReason'));
      await expect(mockContractWrite({ functionName: 'submitRecoveryClaim', args: [owner, ''] })).rejects.toThrow('EmptyReason');
    });

    it('should handle zero address vault', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('ZeroAddress'));
      await expect(mockContractWrite({ functionName: 'submitRecoveryClaim', args: ['0x0000000000000000000000000000000000000000' as Address, 'Lost'] })).rejects.toThrow('ZeroAddress');
    });

    it('should handle double execution', async () => {
      mockContractWrite.mockRejectedValueOnce(new Error('AlreadyExecuted'));
      await expect(mockContractWrite({ functionName: 'executeClaim', args: [1] })).rejects.toThrow('AlreadyExecuted');
    });
  });
});
