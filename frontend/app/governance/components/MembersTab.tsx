        ...
        <div className="bg-[#2A2A2F] border border-[#3A3A3F] rounded-xl p-6">
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold text-[#F5F3E8]">DAO Members & Voting Activity</h2>
              <button
                onClick={() => {
                  const csv =
                ...
                {[{ key: "score", label: "By Score" }, { key: "votes", label: "By Votes" }, { key: "participation", label: "By Participation" }].map(({ key, label }) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-bold transition-all ${
                      sortBy === key ? "bg-[#00F0FF] text-[#1A1A1D]" : "bg-[#1A1A1D] text-[#A0A0A5] hover:text-[#00F0FF]"
                    }`}
                  >
                    {label}
                  </button>
              ...
              <tbody>
                {filteredMembers.map((member, idx) => (
                  <tr key={member.address} className="border-b border-[#3A3A3F] hover:bg-[#1A1A1D] transition-colors">
                    ...
                    <td className="py-4 px-4 text-right">
                      <span className={member.participation >= 85 ? "text-[#50C878]" : "text-[#A0A0A5]"}>{member.participation}%</span>
                    </td>
                    <td className="py-4 px-4 text-right">
                      <span className={member.fatigue > 40 ? "text-[#FFA500]" : "text-[#A0A0A5]"}>{member.fatigue}%</span>
                    </td>
