...
interface ISeerViewTarget {
    ...
    function endorsements(
        address subject,
        address endorser
    ) external view returns (uint64 expiry, uint16 weight, uint64 timestamp);
...
contract SeerView {
    ...
    ) external view returns (uint16[] memory scores) {
        ISeerViewTarget target = ISeerViewTarget(seer);
    ...
    function getTrustLevel(
        ...
        external
        view
        returns (uint8 level, string memory levelName, bool canVote, bool canBeMerchant)
    {
        ISeerViewTarget target = ISeerViewTarget(seer);
        ...
        )
    {
        ...
        if (vault != address(0)) {
            try IEcosystemSchedulerView(vault).checkUpkeep("") returns (
                bool needed,
