...
| `DeployPhase3Peripherals` | n/a for V1 | **Moved to `contracts/future/` 2026-05-20.** Helper that pulls in `contracts/future/BridgeSecurityModule.sol`. Deferred — V1 uses the inline `VFIDEPriceOracle` deploy in Layer 6 instead. Moving to `future/` removes its compilation dependency on `BridgeSecurityModule` from the V1 build set and prevents accidental deploys of the helper on a mainnet bootstrap script. |
