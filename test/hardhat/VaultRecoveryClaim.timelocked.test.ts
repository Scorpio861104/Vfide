...
describe('VaultRecoveryClaim: Timelocked Module Updates', { concurrency: 1 }, () => {
  ...
  it('prevents concurrent vaultHub changes while one is pending', async () => {
    const { contract, replacementHub, replacementRegistry } =
      await deployVaultRecoveryClaimFixture();

    await contract.setVaultHub(replacementHub.address);

    await assert.rejects(
      async () => contract.setVaultHub(replacementRegistry.address),
      /ModuleChangePending|revert/
    );
  });
  ...
  it('applies vaultRegistry update after 48h timelock completes', async () => {
    const { contract, owner, replacementRegistry, ethers } =
      await deployVaultRecoveryClaimFixture();
  ...
  it('prevents concurrent vaultRegistry changes while one is pending', async () => {
    ...
    await assert.rejects(
      async () => contract.setVaultRegistry(replacementHub.address),
      /ModuleChangePending|revert/
    );
  });
  ...
  it('rejects zero address proposals', async () => {
    ...
    await assert.rejects(
      async () => contract.setVaultHub(ethers.ZeroAddress),
      /ZeroAddress|revert/
    );
    await assert.rejects(
      async () => contract.setVaultRegistry(ethers.ZeroAddress),
      /ZeroAddress|revert/
    );
  });

  it('prevents non-owner governance actions', async () => {
    const { contract, replacementHub, replacementRegistry, outsider, ethers } =
      await deployVaultRecoveryClaimFixture();

    await assert.rejects(
      async () => contract.connect(outsider).setVaultHub(replacementHub.address),
      /Ownable|not owner|revert/
    );
    ...
    await assert.rejects(
      async () => contract.connect(outsider).applyVaultRegistry(),
      /Ownable|not owner|revert/
    );
  });
});
