...
describe('DAO admin transfer guardrails', function () {
  async function deployFixture() {
    ...
    const TimelockStub = await ethers.getContractFactory(
      'test/contracts/helpers/DAOAdminTimelockStub.sol:DAOAdminTimelockStub'
    );
  ...
  it('requires pending admin to accept transfer', async function () {
    ...
    await expect(f.dao.connect(f.other).acceptAdmin()).to.be.revertedWithCustomError(
      f.dao,
      'DAO_NotPendingAdmin'
    );
  ...
  it('allows timelock to cancel pending admin transfer', async function () {
    ...
    await expect(f.dao.connect(f.nextAdmin).acceptAdmin()).to.be.revertedWithCustomError(
      f.dao,
      'DAO_NotPendingAdmin'
    );
  });
