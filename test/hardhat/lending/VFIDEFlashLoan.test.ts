...
describe('VFIDEFlashLoan', function () {
  async function deployFixture() {
    ...
    try {
      const Token = await ethers.getContractFactory(
        'test/contracts/helpers/Stubs.sol:ExemptableMintableTokenStub'
      );
  ...
  describe('Flash Loan Execution', function () {
    ...
    it('should reject loan exceeding lender balance', async function () {
      ...
      await expect(
        flashLoan
          .connect(borrower)
          .flashLoan(lender1.address, borrower.address, ethers.parseEther('200'), 100, '0x')
    ...
    it('should reject when paused', async function () {
      ...
      await expect(
        flashLoan
          .connect(borrower)
          .flashLoan(lender1.address, borrower.address, ethers.parseEther('50'), 100, '0x')
    ...
    it('should reject flash loan when lender fee exceeds borrower maxFeeBps', async function () {
      ...
      await expect(
        flashLoan
          .connect(borrower)
          .flashLoan(lender1.address, borrower.address, ethers.parseEther('10'), 10, '0x')
  ...
  describe('DAO Administration', function () {
    it('should reject non-DAO calls', async function () {
      ...
      await expect(flashLoan.connect(lender1).setPaused(true)).to.be.revertedWithCustomError(
        flashLoan,
        'FL_NotDAO'
      );
    });
