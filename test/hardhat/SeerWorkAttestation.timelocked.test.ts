...
async function seerWorkAttestationFixture() {
  ...
  const [admin, dao, merchant, bridge, social, panic, workPayment, outsider, altDao, altMerchant] =
...
describe(
  'SeerWorkAttestation: Timelocked Protocol Contract Updates',
  { concurrency: 1, timeout: 120000 },
  () => {
    it('allows admin to propose protocol contracts update with 48h timelock', async () => {
      const { contract, dao, merchant, bridge, social, panic, workPayment, ethers } =
        await deploySeerWorkAttestationFixture();
    ...
    it('prevents protocol contracts update before 48h timelock elapses', async () => {
      const { contract, dao, merchant, bridge, social, panic, workPayment } =
        await deploySeerWorkAttestationFixture();
    ...
    it('prevents non-admin proposal, apply, and cancel actions', async () => {
      const { contract, dao, merchant, bridge, social, panic, workPayment, outsider, ethers } =
        await deploySeerWorkAttestationFixture();
      ...
      await assert.rejects(
        async () => contract.connect(outsider).cancelProtocolContractsChange(),
        /AccessControl|revert/
      );
      ...
      await assert.rejects(
        async () => contract.connect(outsider).applyProtocolContracts(),
        /AccessControl|revert/
      );
    });
    ...
    it('rejects apply and cancel when no change is pending', async () => {
      ...
      await assert.rejects(
        async () => contract.cancelProtocolContractsChange(),
        /NoPendingChange|revert/
      );
    });

    it('allows canceling and replacing a pending proposal', async () => {
      const { contract, dao, merchant, bridge, social, panic, workPayment, altDao, ethers } =
        await deploySeerWorkAttestationFixture();
    ...
    it('allows replacement of pending change via cancel + new proposal', async () => {
      const { ethers } = await getDefaultConnection();
      const [admin, dao1, merchant1, bridge1, social1, panic1, workPayment1, dao2] =
        await ethers.getSigners();
