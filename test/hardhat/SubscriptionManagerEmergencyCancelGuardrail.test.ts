...
describe('SubscriptionManager emergency cancel guardrail', () => {
  it('blocks emergencyCancel unless emergency breaker is halted', async () => {
    ...
    await manager
      .connect(subscriber)
      .createSubscription(merchant.address, ethers.ZeroAddress, 1n, 3600n, 'test subscription');
  ...
  it('emits SeerRewardFailed when reward calls revert without blocking payment', async () => {
    ...
    const Seer = await ethers.getContractFactory(
      'test/contracts/mocks/InterfaceMocks.sol:RevertingSeerRewardMock'
    );
