...
describe('EmergencyControl recovery', { concurrency: 1, timeout: 120000 }, () => {
  it('queues module changes behind a timelock before applying them', async () => {
    ...
    await control
      .connect(dao)
      .setModules(
        replacementDao.address,
        await replacementBreaker.getAddress(),
        ethers.ZeroAddress
      );

    await assert.rejects(() => control.connect(dao).applyModules(), /modules timelock|revert/);
  ...
  it('requires committee supermajority to cancel recovery proposals', async () => {
    ...
    const Breaker = await ethers.getContractFactory(
      'test/contracts/mocks/InterfaceMocks.sol:EmergencyBreakerStatefulMock'
    );
    const breaker = await Breaker.deploy();
    await breaker.waitForDeployment();
    ...
    const Target = await ethers.getContractFactory(
      'test/contracts/mocks/InterfaceMocks.sol:RecoveryOwnableTargetMock'
    );
    const target = await Target.deploy(dao.address);
    await target.waitForDeployment();
    await target.setEmergencyController(await control.getAddress());

    const proposeTx = await control
      .connect(m1)
      .proposeRecovery(await target.getAddress(), replacementOwner.address);
    const proposeReceipt = await proposeTx.wait();
    const proposedLog = (proposeReceipt?.logs as any[]).find(
      (log) => log.fragment?.name === 'RecoveryProposed'
    );
