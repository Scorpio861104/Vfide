...
async function queueTxAndGetId(
  timelock: any,
  admin: any,
  target: string,
  data: string
...
describe('DAOTimelock admin-rotation execution flow hardening', () => {
  ...
  it('#207: DAO.executeTimelockTx reverts for not-yet-ripe tx', async () => {
    ...
    const pendingSetDelayTxId = await queueTxAndGetId(
      timelock,
      tempAdmin,
      timelockAddr,
      pendingSetDelayData
  ...
  it('#210: requeueExpired preserves DAO proposal tracking mapping', async () => {
    ...
    const oldTx = await timelock
      .connect(tempAdmin)
      .queueTxFromDAO(timelockAddr, 0n, setDelayData, daoProposalId);
    const oldReceipt = await oldTx.wait();
    const oldQueuedEvent = (oldReceipt?.logs as any[]).find(
      (log: any) => log.fragment?.name === 'Queued'
    );
    ...
    const newQueuedEvent = (requeueReceipt?.logs as any[]).find(
      (log: any) => log.fragment?.name === 'Queued'
    );
  ...
  it('#217: setDelay enforces ABSOLUTE_MIN_DELAY floor', async () => {
    ...
    const timelockAddr = await timelock.getAddress();
    const setDelayBelowAbsoluteMin = timelock.interface.encodeFunctionData('setDelay', [
      13 * 60 * 60,
    ]);
    const setDelayTxId = await queueTxAndGetId(
      timelock,
      tempAdmin,
      timelockAddr,
      setDelayBelowAbsoluteMin
    );
  ...
  it('#220: execute does not brick when panicGuard.globalRisk reverts', async () => {
    ...
    const timelockAddr = await timelock.getAddress();
    const setPanicGuardData = timelock.interface.encodeFunctionData('setPanicGuard', [
      await revertingGuard.getAddress(),
    ]);
    const setPanicGuardTxId = await queueTxAndGetId(
      timelock,
      tempAdmin,
      timelockAddr,
      setPanicGuardData
  ...
  it('#225: canRotateSecondaryExecutor provides safe preflight guidance', async () => {
    ...
    const [okContract, reasonContract] = await timelock.canRotateSecondaryExecutor(
      await secondaryCandidate.getAddress()
    );
