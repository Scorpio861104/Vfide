...
describe('MerchantPortal.payWithIntent edge cases', () => {
  async function fixture() {
    ...
    const SessionKeyManager = await ethers.getContractFactory(
      'test/contracts/helpers/Stubs.sol:SessionKeyManagerStub'
    );
  ...
  it('executes happy-path payment and increments vault nonce', async () => {
    const { portal, token, customerVault, merchant, buildIntent, signIntent } =
      await deployFixture();
  ...
  it('enforces SessionKeyManager spend gate when configured', async () => {
    const { portal, sessionKeyManager, dao, merchant, buildIntent, signIntent } =
      (await deployFixture()) as any;
