...
describe('EcosystemVault (EV-07: payExpense accounting order)', { concurrency: 1 }, () => {
  ...
  it('decrements operationsPool before fallback VFIDE transfer', async () => {
    const { ethers, owner, manager, recipient, rewardToken, stableToken, vault } =
      await deployVaultHarness();
    ...
    await vault
      .connect(owner)
      .configureAutoSwap(await inertRouter.getAddress(), await stableToken.getAddress(), true, 100);
  ...
  it('decrements operationsPool before stablecoin transfer from a funded reserve', async () => {
    ...
    await vault
      .connect(owner)
      .configureAutoSwap(await router.getAddress(), await stableToken.getAddress(), true, 100);
  ...
  it('automatically swaps VFIDE to stablecoin for merchant work rewards', async () => {
    ...
    await vault
      .connect(manager)
      .payMerchantWorkReward(recipient.address, 40n, 'merchant stable payout');
