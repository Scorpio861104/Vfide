...
describe('ProofScoreBurnRouter (F-26: only Seer updates score)', () => {
  ...
  it('owner cannot call updateScore directly', async () => {
    ...
    await assert.rejects(() => router.connect(owner).updateScore(user.address), /revert/);
  });
  ...
  it('preview helpers expose the same time-weighted score used for live fee execution', async () => {
    ...
    const previewAccurate = await router.previewFeesAccurate(
      user.address,
      recipient.address,
      1_000_000n
    );
    ...
    const refreshedAccurate = await router.previewFeesAccurate(
      user.address,
      recipient.address,
      1_000_000n
    );
  ...
  it('splits fees from total fee amount to avoid BPS-share rounding drift', async () => {
    ...
    const [burnAmount, sanctumAmount, ecosystemAmount] = await router.computeFees(
      owner.address,
      user.address,
      amount
    );
...
describe('VaultHub (F-20: SecurityHub timelock)', () => {
  ...
  it('rejects legacy setModules and delays VFIDE, ledger, and DAO changes', async () => {
    const { ethers, owner, token, newToken, newLedger, newDao, hub } =
      await deployVaultHubHarness();
...
describe('MerchantPortal (NEW-05: auto-convert safety hold)', () => {
  ...
  it('allows merchants to enable auto-convert only after the swap path is configured', async () => {
    ...
    await assert.rejects(() => portal.connect(merchant).setAutoConvert(true), /revert/);

    await portal.connect(dao).setAcceptedToken(await stablecoin.getAddress(), true);
    await portal
      .connect(dao)
      .setSwapConfig(await router.getAddress(), await stablecoin.getAddress());
...
describe('SeerPolicyGuard (BATCH-06: schedule/cancel flow)', () => {
  ...
  it('allows Seer consume only after policy delay', async () => {
    ...
    await assert.rejects(() => guard.connect(seer).consume(selector, policyClass), /revert/);
...
describe('DAOTimelock (delay hardening)', () => {
  ...
  it('blocks repeated admin emergency delay reductions until the reset window elapses', async () => {
    ...
    await assert.rejects(() => tl.connect(admin).emergencyReduceDelay(30 * 60 * 60), /revert/);
  ...
  it('removes secondary-executed tx ids from queued tracking', async () => {
    ...
    const setSecondaryData = tl.interface.encodeFunctionData('setSecondaryExecutor', [
      secondary.address,
    ]);
    const queueSecondaryTx = await tl
      .connect(admin)
      .queueTx(await tl.getAddress(), 0, setSecondaryData);
  ...
  it('bubbles target revert reason on execute', async () => {
    ...
    const Reverter = await ethers.getContractFactory(
    ...
    await assert.rejects(() => tl.connect(admin).execute(txId), /stub revert reason/);
  });
});
