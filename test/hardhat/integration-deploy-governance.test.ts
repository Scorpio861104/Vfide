...
async function deployGovernanceStack() {
  ...
  const { vaultHub } = await deployVaultHubStack(
    ethers,
...
async function testGovernanceHandover(stack: Awaited<ReturnType<typeof deployGovernanceStack>>) {
  ...
  assert.strictEqual(
    await vaultHub.owner(),
    deployer.address,
    'VaultHub should be owned by deployer initially'
  );
  console.log(
    '    ✅ Pre-transition ownership validated (deployer holds Seer dao + VaultHub owner)'
  );
  ...
  await hubTx.wait();
  assert.strictEqual(
    await vaultHub.pendingOwner(),
    dao.address,
    'VaultHub pendingOwner should be dao after proposal'
  );
  ...
  assert.strictEqual(
    await vaultHub.owner(),
    dao.address,
    'VaultHub owner should be dao post-transition'
  );
  ...
  assert.ok(
    deployerBlocked,
    'Deployer should be blocked from onlyOwner VaultHub functions post-handover'
  );
...
describe('Integration: Deploy + Governance', { concurrency: 1 }, () => {
  it('should deploy governance stack without errors', { timeout: 60_000 }, async () => {
  ...
  it('should validate fee routing wiring', { timeout: 30_000 }, async () => {
    const stack = await deployGovernanceStack();
