...
describe("Inheritance — threat-model coverage", { concurrency: 1 }, () => {
  ...
  it("T-02: propose with non-guardian heir reverts", async () => {
    ...
    await expectRevert(
      f.vault
        .connect(f.owner)
        .proposeInheritanceConfig([f.heir2.address], [commitment]),
    );
  });
  ...
  it("T-09: confirm before cooldown reverts", async () => {
    ...
    await f.vault
      .connect(f.owner)
      .proposeInheritanceConfig([f.heir1.address], [commitment]);
  ...
  it("T-21: cannot initiate inheritance while recovery rotation is pending (INV-2)", async () => {
    ...
    try {
      ...
      await f.vault
        .connect(f.owner)
        .requestWalletRotation(f.heir2.address, 24 * 60 * 60);
      // Now try to initiate inheritance — should revert.
      await expectRevert(
        f.vault
          .connect(f.owner)
          .initiateInheritanceClaim(
            f.ethers.keccak256(f.ethers.toUtf8Bytes("blocked")),
          ),
      );
    } catch (e) {
