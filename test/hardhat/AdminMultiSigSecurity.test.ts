...
describe('AdminMultiSig security hardening', { concurrency: 1, timeout: 120000 }, () => {
  ...
  it('rejects EOA targets at proposal creation time', async () => {
    ...
    try {
      await multisig
        .connect(c0)
        .createProposal(0, outsider.address, '0x12345678', 'attempt no-op call to EOA');
    } catch (error) {
      reverted = true;
  ...
  it('reverts when the underlying proposal execution fails', async () => {
    ...
    await multisig
      .connect(c0)
      .createProposal(0, await multisig.getAddress(), payload, 'set invalid execution gas');
  ...
  it('rejects no-op VFIDE token updates proposed through governance', async () => {
    ...
    await multisig
      .connect(c0)
      .createProposal(0, await multisig.getAddress(), firstPayload, 'set VFIDE token');
    await multisig.connect(c1).approveProposal(0);
    await multisig.connect(c2).approveProposal(0);
    await ethers.provider.send('evm_increaseTime', [24 * 60 * 60 + 1]);
    await ethers.provider.send('evm_mine', []);
    ...
    await multisig
      .connect(c0)
      .createProposal(0, await multisig.getAddress(), secondPayload, 'repeat same VFIDE token');
  ...
  it('allows governance to update the community veto threshold', async () => {
    ...
    await multisig
      .connect(c0)
      .createProposal(0, await multisig.getAddress(), payload, 'raise veto threshold');
    await multisig.connect(c1).approveProposal(0);
    await multisig.connect(c2).approveProposal(0);
