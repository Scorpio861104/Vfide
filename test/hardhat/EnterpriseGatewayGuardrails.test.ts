...
describe('VFIDEEnterpriseGateway guardrails', function () {
  async function deployFixture(useRevertingSeer = false) {
    ...
    if (useRevertingSeer) {
      const AlwaysRevertStub = await ethers.getContractFactory(
        'test/contracts/helpers/Stubs.sol:AlwaysRevertStub'
      );
  ...
  it('prevents rescuing escrowed VFIDE pending orders', async function () {
    ...
    await expect(
      f.gateway
        .connect(f.dao)
        .rescueFunds(await f.token.getAddress(), ethers.parseEther('1'), f.dao.address)
  ...
  it('allows rescuing non-escrow tokens', async function () {
    ...
    await f.stable
      .connect(f.rescuer)
      .transfer(await f.gateway.getAddress(), ethers.parseEther('50'));

    const daoBefore = await f.stable.balanceOf(f.dao.address);
    await f.gateway
      .connect(f.dao)
      .rescueFunds(await f.stable.getAddress(), ethers.parseEther('10'), f.dao.address);
  ...
  it('accepts non-standard ERC20 tokens in createOrder', async function () {
    ...
    const NonStandard = await ethers.getContractFactory(
      'test/contracts/mocks/MockContracts.sol:MockNonStandardERC20'
    );
    const nonStandard = await NonStandard.connect(dao).deploy(
      'Mock USDT',
      'mUSDT',
    ...
    await gateway
      .connect(buyer)
      .createOrder(orderId, ethers.parseEther('25'), 'order-non-standard');
  ...
  it('emits SeerRewardFailed when trust reward call reverts', async function () {
    ...
    await f.gateway
      .connect(f.buyer)
      .createOrder(orderId, ethers.parseEther('25'), 'order-reward-fail');
  ...
  it('uses forceApprove-compatible flow for zero-first tokens', async function () {
    ...
    const ZeroFirst = await ethers.getContractFactory(
      'test/contracts/mocks/MockContracts.sol:MockZeroFirstApproveERC20'
    );
    const token = await ZeroFirst.connect(owner).deploy(
      'Zero First',
      'ZFA',
      ethers.parseEther('1')
    );
    ...
    const ForceApproveHarness = await ethers.getContractFactory(
      'test/contracts/helpers/SafeERC20Harness.sol:ForceApproveHarness'
    );
    ...
    expect(await token.allowance(await harness.getAddress(), await spender.getAddress())).to.equal(
      25n
    );
