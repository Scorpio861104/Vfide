...
describe("ProofLedger", function () {
  ...
  describe("Deployment", function () {
    ...
    it("should revert on zero address DAO", async function () {
      ...
      await expect(ProofLedger.deploy(ethers.ZeroAddress))
        .to.be.revertedWithCustomError(ledger, "TRUST_Zero");
    });
  });
  ...
  describe("logEvent", function () {
    it("should emit EventLog event with all parameters", async function () {
      ...
      await expect(tx)
        .to.emit(ledger, "EventLog")
        .withArgs(who, action, amount, note);
    });
    ...
    it("should handle very large amounts", async function () {
      ...
      await expect(ledger.logEvent(user1.address, "large", largeAmount, "huge"))
        .to.emit(ledger, "EventLog")
        .withArgs(user1.address, "large", largeAmount, "huge");
    });
    ...
    it("should preserve msg.sender in 'by' field", async function () {
      ...
      const event = receipt.logs.find(log => {
        try {
          return ledger.interface.parseLog(log)?.name === "EventLog";
        } catch {
          return false;
        }
      });
  ...
  describe("logSystemEvent", function () {
    it("should emit SystemEvent event with all parameters", async function () {
      const who = user1.address;
    ...
    it("should use current block timestamp", async function () {
      ...
      const event = receipt.logs.find(log => {
        try {
          return ledger.interface.parseLog(log)?.name === "SystemEvent";
        } catch {
          return false;
        }
      });
      const parsedEvent = ledger.interface.parseLog(event);
      // expect(parsedEvent.args.time).to.equal(block.timestamp); // No timestamp in event
    });
    */
  });
  ...
  describe("Edge Cases", function () {
    ...
    it("should handle maximum uint256 amount in logs", async function () {
      await expect(ledger.logEvent(user1.address, "max", ethers.MaxUint256, "max amount"))
        .to.emit(ledger, "EventLog")
        .withArgs(user1.address, "max", ethers.MaxUint256, "max amount");
    });
