...
describe('VFIDEBridge', function () {
  ...
  describe('Deployment', function () {
    ...
    it('rejects zero constructor addresses', async function () {
      const BridgeFactory = await ethers.getContractFactory('VFIDEBridge');
      await expectRevert(
        BridgeFactory.deploy(ethers.constants.AddressZero, endpoint.address, owner.address)
      );
      await expectRevert(
        BridgeFactory.deploy(token.address, ethers.constants.AddressZero, owner.address)
      );
      await expectRevert(
        BridgeFactory.deploy(token.address, endpoint.address, ethers.constants.AddressZero)
      );
    });
  });
  ...
  describe('Bridge guardrails', function () {
    it('reverts on invalid amount', async function () {
      await expectRevert(bridge.connect(user).bridge(REMOTE_CHAIN_ID, user.address, 0, '0x'));
    });
    ...
    it('reverts on zero destination', async function () {
      await expectRevert(
        bridge
          .connect(user)
          .bridge(REMOTE_CHAIN_ID, ethers.constants.AddressZero, ethers.utils.parseEther('1'), '0x')
      );
    });
    ...
    it('reverts when remote is not trusted', async function () {
      await expectRevert(
        bridge
          .connect(user)
          .bridge(REMOTE_CHAIN_ID, user.address, ethers.utils.parseEther('1'), '0x')
      );
    });
    ...
    it('enforces max bridge amount', async function () {
      ...
      await expectRevert(
        bridge
          .connect(user)
          .bridge(REMOTE_CHAIN_ID, user.address, ethers.utils.parseEther('201'), '0x')
      );
    });
    ...
    it('enforces the daily aggregate bridge limit', async function () {
      ...
      await bridge
        .connect(user)
        .bridge(REMOTE_CHAIN_ID, user.address, ethers.utils.parseEther('100'), '0x');
      await expectRevert(
        bridge
          .connect(user)
          .bridge(REMOTE_CHAIN_ID, user.address, ethers.utils.parseEther('100'), '0x')
