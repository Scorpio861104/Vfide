...
describe("VFIDEToken exhaustive branch tests", function () {
  ...
  it("stakingRewardsVault, sanctumVault, and ecosystemOpsVault are set correctly in constructor", async function () {
    const stakingVault = ethers.Wallet.createRandom().address;
    const sanctumVault = ethers.Wallet.createRandom().address;
    const opsVault = ethers.Wallet.createRandom().address;

    const Token = await ethers.getContractFactory("VFIDEToken");
    const tokenWithVaults = await Token.deploy(
      vest.target,
      ethers.ZeroAddress,
      ethers.ZeroAddress,
      ethers.ZeroAddress,
    ...
    await tokenWithVaults.waitForDeployment();

    expect(await tokenWithVaults.stakingRewardsVault()).to.equal(stakingVault);
    expect(await tokenWithVaults.sanctumVault()).to.equal(sanctumVault);
    expect(await tokenWithVaults.ecosystemOpsVault()).to.equal(opsVault);
  });
});
