...
describe("GovernanceHooks", function () {
  ...
  beforeEach(async function () {
    ...
    const LedgerMock = await ethers.getContractFactory("contracts-min/ProofLedger.sol:ProofLedger");
  ...
  describe("Deployment", function () {
    ...
    it("should emit ModulesSet on deployment", async function () {
      const GovernanceHooks = await ethers.getContractFactory("GovernanceHooks");
      await expect(GovernanceHooks.deploy(await ledger.getAddress(), await seer.getAddress()))
        .to.emit(GovernanceHooks, "ModulesSet")
  ...
  describe("setModules", function () {
    ...
    it("should allow anyone to call setModules", async function () {
      const newLedger = ethers.Wallet.createRandom().address;
      await hooks.connect(voter).setModules(newLedger, await seer.getAddress());
      expect(await hooks.ledger()).to.equal(newLedger);
    });
  });
  ...
  describe("onFinalized", function () {
    ...
    it("should log system event when ledger is set (passed)", async function () {
      await expect(hooks.onFinalized(1, true))
        .to.emit(ledger, "LogSystem");
    });
