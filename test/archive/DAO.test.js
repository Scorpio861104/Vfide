...
describe('DAO Contract Tests', function () {
  ...
  beforeEach(async function () {
    ...
    dao = await DAO.deploy(admin.address, admin.address, await seer.getAddress(), await vaultHub.getAddress(), ethers.ZeroAddress);
    
    // Deploy timelock with DAO as admin
    const Timelock = await ethers.getContractFactory('DAOTimelock');
    timelock = await Timelock.deploy(await dao.getAddress());
  ...
  describe('Parameter Updates', function () {
    it('should allow admin to update voting period', async function () {
      ...
      expect(await dao.quorum()).to.equal(60);
    });
