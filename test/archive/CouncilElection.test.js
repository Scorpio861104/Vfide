...
describe('CouncilElection Contract Tests', function () {
  ...
  beforeEach(async function () {
    ...
    council = await Council.deploy(dao.address, seer.target, vaultHub.target, ledger.target);

    await seer.setMin(100);
    await seer.setScore(candidate1.address, 150);
    await seer.setScore(candidate2.address, 160);
  ...
  describe('Deployment', function () {
    ...
    it('should set default term to 180 days', async function () {
      expect(await council.termSeconds()).to.equal(180 * 24 * 60 * 60);
    });

    it('should revert if deploying with zero addresses', async function () {
      const Council = await ethers.getContractFactory('CouncilElection');
      await expect(Council.deploy(ethers.ZeroAddress, seer.target, vaultHub.target, ledger.target))
