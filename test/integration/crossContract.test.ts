...
describe('Integration: Bridge Full Flow', function () {
  ...
  it('should execute bridge → rate limit check → circuit breaker → send', async function () {
    ...
    await expect(circuitBreaker.recordVolume(ethers.utils.parseEther('100'))).to.emit(
      circuitBreaker,
      'VolumeRecorded'
    );
  ...
  it('should trip circuit breaker on excessive bridge volume', async function () {
    ...
    await expect(circuitBreaker.recordVolume(ethers.utils.parseEther('11'))).to.emit(
      circuitBreaker,
...
describe('Integration: Vault Operations Full Flow', function () {
  ...
  it('should execute vault-to-vault transfer with EIP-712 signature', async function () {
    ...
    expect(vaultFactory.interface.getFunction('executeVaultToVaultTransfer')).to.not.equal(
      undefined
    );
  });
...
describe('Integration: Emergency Flow', function () {
  ...
  async function deployFixture() {
    ...
    emergencyControl = await EmergencyControl.deploy(
      owner.address,
      breaker.address,
      ledger.address
    );
  ...
  beforeEach(async function () {
    ({ owner, memberA, memberB, outsider, breaker, emergencyControl, ledger } =
      await loadFixture(deployFixture));
  });

  it('should cascade emergency pause across all contracts', async function () {
    await expect(emergencyControl.daoToggle(true, 'integration emergency')).to.emit(
      emergencyControl,
      'DAOToggled'
    );
  ...
  it('should block all user operations during emergency', async function () {
    ...
    await expect(emergencyControl.connect(outsider).daoToggle(false, 'attempted bypass')).to.be
      .reverted;

    // Cooldown prevents immediate unhalt after activation.
    await expect(emergencyControl.daoToggle(false, 'premature recovery')).to.be.reverted;
  });
